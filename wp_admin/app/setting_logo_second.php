<?php
	...
	if(isset($_POST['form-logo-update'])){			
		...
		if(!empty($_FILES["SYS_SECOND_LOGO"]["name"])) { 
			// Get file info 
			$fileName = basename($_FILES["SYS_SECOND_LOGO"]["name"]); 
			$fileType = pathinfo($fileName, PATHINFO_EXTENSION); 
			 
			// Allow certain file formats 
			$allowTypes = array('jpg','png','jpeg','gif','avif','tiff','eps','raw','indd'); 
			if(in_array($fileType, $allowTypes)){ 
				$image = $_FILES['SYS_SECOND_LOGO']['tmp_name']; 
				$SYS_LOGO = addslashes(file_get_contents($image)); 
			 
				$insert = $conn->query("UPDATE tbl_system_setting SET SYS_SECOND_LOGO='$SYS_LOGO' WHERE SYS_ID='$SYS_ID'");
				if($insert){ 
